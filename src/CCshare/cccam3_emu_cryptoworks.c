// Cryptoworks EMU - portado de OSCam-emu (GPLv3) module-emulator-cryptoworks.c
...
// --- Pesquisa de Chaves ('W' no SoftCam.Key) ---

static int cw_get_key(uint8_t *buf, uint32_t ident, uint8_t key_index, uint32_t key_length)
{
	char key_name[8];

	if ((ident >> 4) == 0xD02A)
	{
		key_index &= 0xFE; // map to even number key indexes
	}
	...
	else if (key_index == 6 && ((ident >> 8) == 0x0D05))
	{
		ident = 0x0D0504; // always use provider 04 system key
	}

	snprintf(key_name, sizeof(key_name), "%.2X", (unsigned int)key_index);

	if (cccam_emu_find_key('W', ident, key_name, 0, buf, key_length) >= (int)key_length)
	{
		return 1;
	}
...
static const uint8_t cw_AND_bit1[8] = { 0x00, 0x40, 0x04, 0x80, 0x21, 0x10, 0x02, 0x08 };
static const uint8_t cw_AND_bit2[8] = { 0x80, 0x08, 0x01, 0x40, 0x04, 0x20, 0x10, 0x02 };
static const uint8_t cw_AND_bit3[8] = { 0x82, 0x40, 0x01, 0x10, 0x00, 0x20, 0x04, 0x08 };
static const uint8_t cw_AND_bit4[8] = { 0x02, 0x10, 0x04, 0x40, 0x80, 0x08, 0x01, 0x20 };
...
static void cw_des_round(uint8_t *d, uint8_t *k)
{
	...
	for (i = 0; i < 8; i++)
	{
		d[0] ^= (cw_AND_bit1[i] & cw_sbox1[des_td[i]]);
		d[1] ^= (cw_AND_bit2[i] & cw_sbox2[des_td[i]]);
		d[2] ^= (cw_AND_bit3[i] & cw_sbox3[des_td[i]]);
		d[3] ^= (cw_AND_bit4[i] & cw_sbox4[des_td[i]]);
	}
...
static void cw_rs_des_subkey(uint8_t *k, uint8_t rotate_counter)
{
	...
	cw_rs_des_key(k, rnd[rotate_counter]);
}
...
static void cw_l2_des(uint8_t *data, uint8_t *key, uint8_t algo)
{
...
static void cw_r2_des(uint8_t *data, uint8_t *key, uint8_t algo)
{
	uint8_t i, k0[22], k1[22];
...
static uint8_t cw_process_nano80(uint8_t *data, uint32_t caid, int32_t provider,
								 uint8_t *opKey, uint8_t nanoLength, uint8_t nano80Algo)
{
	int32_t i, j;
	...
	if (caid == 0x0D00 && provider != 0xA0 && !cw_get_key(k0D00C000, 0x0D00C0, 0, 16))
	{
		return 0;
	}
	...
	cccam_emu_des_ecb3_decrypt(data, key);
	...
	for (i = 8; i + 7 < nanoLength; i += 8)
	{
		...
		cccam_emu_des_ecb3_decrypt(dat1, key);
...
static void cw_signature(const uint8_t *data, uint32_t length, uint8_t *key, uint8_t *signature)
{
	uint32_t i, sigPos;
	int8_t algo, first;
	...
	for (i = 0; i < length; i++)
	{
		...
		if (sigPos > 7)
		{
			if (first)
			{
				cw_l2_des(signature, key, (uint8_t)algo);
			}
	...
	cw_r2_des(signature, key, (uint8_t)algo);
}

static void cw_decrypt_des(uint8_t *data, uint8_t algo, uint8_t *key)
{
	int32_t i;
	uint8_t k[22], t[8];
	...
	else
	{
		...
		for (i = 0; i < 3; i++)
		{
			cw_dec_enc(data, k, algo, (uint8_t)(i & 1));
...
static uint16_t cw_sct_len(const uint8_t *ecm)
{
...
int cccam_emu_cryptoworks_ecm(uint32_t caid, uint8_t *ecm, uint8_t *cw)
{
	...
	uint16_t i, j, ecmLen = cw_sct_len(ecm);
	uint32_t ident;
	...
	ident = (caid << 8) | (uint32_t)provider;

	if (!cw_get_key(key, ident, keyIndex, 16))
	{
		return CCCAM_EMU_KEY_NOT_FOUND;
	}

	if (!cw_get_key(&key[16], ident, 6, 6))
	{
		return CCCAM_EMU_KEY_NOT_FOUND;
	}
	...
	for (i = 8; i + 1 < ecmLen; i += ecm[i + 1] + 2)
	{
		...
		{
			nanoLength = ecm[i + 1];
			newEcmLength = cw_process_nano80(ecm + i + 2, caid, provider, key, nanoLength, nano80Algo);
			...
			return cccam_emu_cryptoworks_ecm(caid, ecm + i + 2 + 3, cw);
		}
	}
	...
	cw_signature(ecm + 5, ecmLen - 15, key, signature);
	...
	for (i = 8; i + 1 < ecmLen; i += ecm[i + 1] + 2)
	{
		switch (ecm[i])
		{
			...
			case 0xDC:
				...
				{
					cw_decrypt_des(&ecm[i + 2 + j], ecm[5], key);
				}
				break;
		...
		switch (ecm[i])
		{
			case 0xDB:
				if (i + 2 + ecm[i + 1] <= ecmLen && ecm[i + 1] == 16)
				{
					...
					cccam_log(LOG_DEBUG, "EMU Cryptoworks: CW obtida (ident %06X, chave %.2X)",
							  ident, keyIndex);
					return CCCAM_EMU_OK;
				}
				break;
