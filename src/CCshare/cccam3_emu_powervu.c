// PowerVU EMU (ECM, modo CSA) - portado de OSCam-emu (GPLv3)
// module-emulator-powervu.c (apenas o caminho de ECM com chaves 'P'
// do SoftCam.Key; sem EMM AU nem stream server).
...
#define PVU_CONVCW_VID_ECM 0x80
#define PVU_CONVCW_HSD_ECM 0x40
#define PVU_CONVCW_A1_ECM  0x20
#define PVU_CONVCW_A2_ECM  0x10
#define PVU_CONVCW_A3_ECM  0x08
#define PVU_CONVCW_A4_ECM  0x04
#define PVU_CONVCW_UTL_ECM 0x02
#define PVU_CONVCW_VBI_ECM 0x01

// --- Utilitarios ---

static uint8_t pvu_get_bit(uint8_t val, uint8_t bitnb)
{
	return (val >> bitnb) & 1;
}

static uint8_t pvu_set_bit(uint8_t val, uint8_t bitnb, uint8_t biton)
{
	return biton ? (uint8_t)(val | (1 << bitnb)) : (uint8_t)(val & ~(1 << bitnb));
}

static uint16_t pvu_b2i(uint8_t n, const uint8_t *b)
{
	switch (n)
	{
		case 2:
		...
		case 3:
			return (uint16_t)((b[0] << 16) | (b[1] << 8) | b[2]);
		case 4:
			return (uint16_t)((b[0] << 24) | (b[1] << 16) | (b[2] << 8) | b[3]);
		default:
			return 0;
	}
}

static uint8_t pvu_crc8_calc(uint8_t *data, int len)
{
	int i;
	uint8_t crc = 0;
...
// CRC-32/MPEG-2 (init 0xFFFFFFFF, sem XOR final)
static uint32_t pvu_ccitt32_crc(uint8_t *data, uint32_t len)
{
	...
	while (len--)
	{
		crc = (crc << 8) ^ tab[((crc >> 24) ^ *data++) & 0xFF];
	}
	return crc;
}

static void pvu_pad_data(uint8_t *data, int len, uint8_t *dataPadded)
{
	int i;
	uint8_t pad[] =
	{
...
static void pvu_hash_mode_01_custom_md5(uint8_t *data, uint8_t *hash)
{
	int i, j, s;
	...
	uint8_t tIdxInit[] = { 0, 1, 5, 0 };
	uint8_t tIdxIncr[] = { 1, 5, 3, 7 };

	uint32_t h[] = { 0xEAD81D2E, 0xCE4DC6E9, 0xF9B5C301, 0x10325476 };
	uint32_t dataLongs[16];

	for (i = 0; i < 16; i++)
	{
		dataLongs[i] = ((uint32_t)data[4 * i + 0] << 0) + ((uint32_t)data[4 * i + 1] << 8) +
	...
	for (i = 0; i < 4; i++)
	{
		hash[4 * i + 0] = (uint8_t)(h[i] >> 0);
		hash[4 * i + 1] = (uint8_t)(h[i] >> 8);
		hash[4 * i + 2] = (uint8_t)(h[i] >> 16);
		hash[4 * i + 3] = (uint8_t)(h[i] >> 24);
	}
}

static void pvu_hash_mode_02(uint8_t *data, uint8_t *hash)
{
	int i;
	...
	for (i = 0; i < 16; i++)
	{
		dataLongs[i] = ((uint32_t)data[4 * i + 0] << 24) + ((uint32_t)data[4 * i + 1] << 16) +
	...
	for (i = 0; i < 4; i++)
	{
		hash[4 * i + 0] = (uint8_t)(dataLongs[i] >> 24);
		hash[4 * i + 1] = (uint8_t)(dataLongs[i] >> 16);
		hash[4 * i + 2] = (uint8_t)(dataLongs[i] >> 8);
		hash[4 * i + 3] = (uint8_t)(dataLongs[i] >> 0);
	}
}

static void pvu_hash_mode_03(uint8_t *data, uint8_t *hash)
{
	int i, j, k, s, s2, tmp;
	uint32_t a, b, c, d, f = 0, g;
	...
	for (i = 0; i < 4; i++)
	{
		hash[4 * i + 0] = (uint8_t)(result[i] >> 0);
		hash[4 * i + 1] = (uint8_t)(result[i] >> 8);
		hash[4 * i + 2] = (uint8_t)(result[i] >> 16);
		hash[4 * i + 3] = (uint8_t)(result[i] >> 24);
	}
}

static const uint8_t pvu_table04[] =
{
...
static const uint8_t pvu_table05[] =
{
...
static const uint8_t pvu_table06[] =
{
...
static const uint8_t pvu_table07[] =
{
...
static const uint8_t pvu_table08[] =
{
...
static const uint8_t pvu_table09[] =
{
...
static const uint8_t pvu_table0A[] =
{
...
static void pvu_hash_modes_04_to_0A_tables(uint8_t *data, uint8_t *hash, const uint8_t *table)
{
	int i;
...
static const uint8_t pvu_table0F[] = { 0xC7, 0x45, 0x15, 0x71, 0x61, 0x07, 0x05, 0x47 };
static const uint8_t pvu_table10[] = { 0x0F, 0x47, 0x2B, 0x6C, 0xAD, 0x0F, 0xB3, 0xEA };
static const uint8_t pvu_table11[] = { 0xB1, 0x46, 0xD1, 0x66, 0x5D, 0x28, 0x59, 0xD2 };
static const uint8_t pvu_table12[] = { 0x0B, 0x4B, 0xD7, 0x68, 0x5F, 0xAD, 0x4B, 0xBB };
static const uint8_t pvu_table13[] = { 0x4F, 0x4E, 0xE1, 0x6A, 0x21, 0xD3, 0xF7, 0xA6 };
static const uint8_t pvu_table14[] = { 0xDD, 0x39, 0xB9, 0x65, 0x03, 0x91, 0xF1, 0xAC };
static const uint8_t pvu_table15[] = { 0x3F, 0x50, 0xB5, 0x6F, 0x37, 0xC9, 0x13, 0x5D };
static const uint8_t pvu_table16[] = { 0xF9, 0x5C, 0xFD, 0x72, 0x19, 0x42, 0x23, 0x6B };
static const uint8_t pvu_table17[] = { 0xDF, 0x60, 0x93, 0x64, 0x33, 0x16, 0xB3, 0x8A };
static const uint8_t pvu_table18[] = { 0x09, 0x64, 0x5F, 0x6B, 0xFB, 0x21, 0x19, 0xE4 };

static void pvu_hash_modes_0F_to_18_tables(uint8_t *data, uint8_t *hash, const uint8_t *table)
{
	int i;
	uint32_t t[4], tmp;
...
static const uint8_t pvu_table19[] = { 0x02, 0x03, 0x05, 0x10 };
static const uint8_t pvu_table1A[] = { 0x01, 0x05, 0x08, 0x10 };
static const uint8_t pvu_table1B[] = { 0x03, 0x07, 0x08, 0x10 };
static const uint8_t pvu_table1C[] = { 0x03, 0x05, 0x0A, 0x10 };
static const uint8_t pvu_table1D[] = { 0x03, 0x07, 0x0A, 0x10 };
static const uint8_t pvu_table1E[] = { 0x01, 0x05, 0x0B, 0x10 };
static const uint8_t pvu_table1F[] = { 0x06, 0x07, 0x0B, 0x10 };
static const uint8_t pvu_table20[] = { 0x01, 0x08, 0x0B, 0x10 };
static const uint8_t pvu_table21[] = { 0x01, 0x07, 0x0C, 0x10 };
static const uint8_t pvu_table22[] = { 0x05, 0x0B, 0x0C, 0x10 };

static void pvu_hash_modes_19_to_27_tables_3(uint8_t *data, uint8_t *hash, const uint8_t *table)
{
	int i;
	uint8_t val, it[4];
	...
	for (i = 0; i < 16; i++)
	{
		val = (uint8_t)(((seed >> it[0]) ^ (seed >> it[1]) ^ (seed >> it[2]) ^ (seed >> it[3])) & 0x01);
		...
		tmp = seed + (uint16_t)(data[i] << 8) + data[i + 32];
		...
		tmp = (uint16_t)(tmp + seed + (data[i + 16] << 8) + data[i + 48]);
...
static void pvu_create_hash(uint8_t *data, int len, uint8_t *hash, int mode)
{
	...
	pvu_pad_data(data, len, dataPadded);

	switch (mode)
	{
		case 1:
			pvu_hash_mode_01_custom_md5(dataPadded, hash);
			break;

		case 2:
			pvu_hash_mode_02(dataPadded, hash);
			break;

		case 3:
			pvu_hash_mode_03(dataPadded, hash);
			break;

		case 4:
			pvu_hash_modes_04_to_0A_tables(dataPadded, hash, pvu_table04);
			break;

		case 5:
			pvu_hash_modes_04_to_0A_tables(dataPadded, hash, pvu_table05);
			break;

		case 6:
			pvu_hash_modes_04_to_0A_tables(dataPadded, hash, pvu_table06);
			break;

		case 7:
			pvu_hash_modes_04_to_0A_tables(dataPadded, hash, pvu_table07);
			break;

		case 8:
			pvu_hash_modes_04_to_0A_tables(dataPadded, hash, pvu_table08);
			break;

		case 9:
			pvu_hash_modes_04_to_0A_tables(dataPadded, hash, pvu_table09);
			break;

		case 10:
			pvu_hash_modes_04_to_0A_tables(dataPadded, hash, pvu_table0A);
			break;

		case 15:
			pvu_hash_modes_0F_to_18_tables(dataPadded, hash, pvu_table0F);
			break;

		case 16:
			pvu_hash_modes_0F_to_18_tables(dataPadded, hash, pvu_table10);
			break;

		case 17:
			pvu_hash_modes_0F_to_18_tables(dataPadded, hash, pvu_table11);
			break;

		case 18:
			pvu_hash_modes_0F_to_18_tables(dataPadded, hash, pvu_table12);
			break;

		case 19:
			pvu_hash_modes_0F_to_18_tables(dataPadded, hash, pvu_table13);
			break;

		case 20:
			pvu_hash_modes_0F_to_18_tables(dataPadded, hash, pvu_table14);
			break;

		case 21:
			pvu_hash_modes_0F_to_18_tables(dataPadded, hash, pvu_table15);
			break;

		case 22:
			pvu_hash_modes_0F_to_18_tables(dataPadded, hash, pvu_table16);
			break;

		case 23:
			pvu_hash_modes_0F_to_18_tables(dataPadded, hash, pvu_table17);
			break;

		case 24:
			pvu_hash_modes_0F_to_18_tables(dataPadded, hash, pvu_table18);
			break;

		case 25:
			pvu_hash_modes_19_to_27_tables_3(dataPadded, hash, pvu_table19);
			break;

		case 26:
			pvu_hash_modes_19_to_27_tables_3(dataPadded, hash, pvu_table1A);
			break;

		case 27:
			pvu_hash_modes_19_to_27_tables_3(dataPadded, hash, pvu_table1B);
			break;

		case 28:
			pvu_hash_modes_19_to_27_tables_3(dataPadded, hash, pvu_table1C);
			break;

		case 29:
			pvu_hash_modes_19_to_27_tables_3(dataPadded, hash, pvu_table1D);
			break;

		case 30:
			pvu_hash_modes_19_to_27_tables_3(dataPadded, hash, pvu_table1E);
			break;

		case 31:
			pvu_hash_modes_19_to_27_tables_3(dataPadded, hash, pvu_table1F);
			break;

		case 32:
			pvu_hash_modes_19_to_27_tables_3(dataPadded, hash, pvu_table20);
			break;

		case 33:
			pvu_hash_modes_19_to_27_tables_3(dataPadded, hash, pvu_table21);
			break;

		case 34:
			pvu_hash_modes_19_to_27_tables_3(dataPadded, hash, pvu_table22);
			break;

		default:
			cccam_log(LOG_DEBUG, "EMU PowerVU: novo modo de hash [%d] em uso", mode);
			break;
	}
}
...
static void pvu_create_hash_mode_03(uint8_t *data, uint8_t *hash)
{
	int i, j, c;
	...
	for (c = 0; c < 12; c++)
	{
		...
		for (j = 0; j < 8; j++)
		{
			...
			for (i = 0; i < 8; i++)
			{
				buffer2[j] ^= buffer1[i] * (uint8_t)(j * i + 1);
			}
		}
...
// --- Hash mode 04 (para unmask) ---

static void pvu_hash_04_add(uint32_t *buffer, int a, int b, int c, int d, int e, int f)
{
...
static void pvu_hash_04_shift(uint32_t *buffer, int a, int b, uint8_t shift)
{
...
static void pvu_hash_04_xor(uint32_t *buffer, int a, int b, int c, int d)
{
	buffer[a] ^= buffer[b];
	buffer[c] ^= buffer[d];
}

static void pvu_hash_04_swap(uint32_t *buffer, int a, int b)
{
...
static void pvu_hash_04_core(uint32_t *buffer)
{
	pvu_hash_04_add(buffer, 0, 6, 7, 1, 7, 6);
	pvu_hash_04_shift(buffer, 5, 4, 0x0D);
	pvu_hash_04_xor(buffer, 4, 2, 5, 3);
	pvu_hash_04_swap(buffer, 7, 6);
	pvu_hash_04_add(buffer, 6, 2, 3, 7, 3, 2);
	pvu_hash_04_shift(buffer, 1, 0, 0x10);
	pvu_hash_04_xor(buffer, 0, 4, 1, 5);
	pvu_hash_04_add(buffer, 6, 2, 3, 7, 7, 6);
	pvu_hash_04_shift(buffer, 1, 0, 0x15);
	pvu_hash_04_add(buffer, 6, 0, 1, 7, 1, 0);
	pvu_hash_04_xor(buffer, 2, 4, 3, 5);
	pvu_hash_04_shift(buffer, 5, 4, 0x11);
	pvu_hash_04_xor(buffer, 4, 2, 5, 3);
	pvu_hash_04_swap(buffer, 3, 2);
}

static void pvu_create_hash_mode_04(uint8_t *data, uint8_t *hash)
{
	int i, j;
	...
	for (j = 0; j < 64; j += 8)
	{
		d0 = ((uint32_t)data[j + 3] << 24) + ((uint32_t)data[j + 2] << 16) + ((uint32_t)data[j + 1] << 8) + data[j + 0];
		d1 = ((uint32_t)data[j + 7] << 24) + ((uint32_t)data[j + 6] << 16) + ((uint32_t)data[j + 5] << 8) + data[j + 4];
...
// --- Criacao de dados (unmask/CW modes 03 e 04) ---

static void pvu_create_data_cw_mode_03(uint8_t *seed, int lenSeed, uint8_t *basecw,
...
static void pvu_create_data_cw_mode_04(uint8_t *seed, int lenSeed, uint8_t *basecw,
										uint8_t val, uint8_t *ecmBody, uint8_t *data)
{
	uint8_t padding[] =
	{
...
static void pvu_create_data_unmask_mode_03(uint8_t *ecmBody, uint8_t *data)
{
	uint8_t padding[] =
	{
...
static void pvu_create_data_unmask_mode_04(uint8_t *ecmBody, uint8_t *data)
{
	uint8_t padding[] =
	{
...
static uint8_t pvu_get_mode_cw(uint8_t *extraData)
{
	uint64_t data = ((uint32_t)extraData[0] << 24) + (extraData[1] << 16) + (extraData[2] << 8) + extraData[3];
	uint64_t t1 = (data * 0x76E9DEA7ULL) >> 50;
	uint64_t t2 = (t1 * 0x51EB851FULL) >> 36;
	uint64_t t3 = t2 * 0x32;
	return (uint8_t)(t1 - t3);
}

static uint8_t pvu_get_mode_unmask(uint8_t *extraData)
{
	uint64_t data = ((uint32_t)extraData[0] << 24) + (extraData[1] << 16) + (extraData[2] << 8) + extraData[3];
	uint64_t t1 = (data * 0xB9CD6BE5ULL) >> 45;
	uint64_t t2 = (t1 * 0x51EB851FULL) >> 36;
	uint64_t t3 = t2 * 0x32;
	return (uint8_t)(t1 - t3);
}

static void pvu_create_data_ecm_emm(uint8_t *emmEcm, uint8_t *pos, int lenHeader, int len, uint8_t *data)
{
	int i;
...
static uint8_t pvu_create_data_cw(uint8_t *seed, uint8_t lenSeed, uint8_t *baseCw,
								  uint8_t val, uint8_t *seedEcmCw, uint8_t *data)
{
	...
	return (uint8_t)(lenSeed + 7 + 1 + 0x10);
}

static uint8_t pvu_unmask_ecm(uint8_t *ecm, uint8_t *seedEcmCw, uint8_t *modeCW)
{
	int i, l;
	...
	*modeCW = 0;
	if (extraBytesLen > 0)
	{
		*modeCW = pvu_get_mode_cw(ecm + 10);
	}

	hashModeCw = ecm[28 + extraBytesLen] ^ pvu_crc8_calc(seedEcmCw, 16);

	pvu_create_data_ecm_emm(ecm, sourcePos, startOffset, 24, data);

	hashModeEcm = ecm[8] ^ pvu_crc8_calc(data, 24);

	if (extraBytesLen > 0)
	{
		modeUnmask = pvu_get_mode_unmask(ecm + 10);
	}
	...
	if (modeUnmask == 0x03)
	{
		...
		pvu_create_data_unmask_mode_03(ecm + startOffset, data);
	...
	else if (modeUnmask == 0x04)
	{
		...
		pvu_create_data_unmask_mode_04(ecm + startOffset, data);
		pvu_create_hash_mode_04(data, mask);
	...
	{
		pvu_create_hash(data, 24, mask, hashModeEcm);
	...
	crc = pvu_ccitt32_crc(ecm, l);

	ecm[l + 0] = (uint8_t)(crc >> 24);
	ecm[l + 1] = (uint8_t)(crc >> 16);
	ecm[l + 2] = (uint8_t)(crc >> 8);
	ecm[l + 3] = (uint8_t)(crc >> 0);
...
static void pvu_create_cw(uint8_t *seed, uint8_t lenSeed, uint8_t *baseCw, uint8_t val,
						  uint8_t *seedEcmCw, uint8_t *cw, int modeDesCsa,
						  int hashMode, int modeCW, uint8_t *ecmBody)
{
	int i;
	...
	if (modeCW == 0x03)
	{
		pvu_create_data_cw_mode_03(seed, lenSeed, baseCw, val, ecmBody, data);
		pvu_create_hash_mode_03(data, hash);
	...
	{
		pvu_create_data_cw_mode_04(seed, lenSeed, baseCw, val, ecmBody, data);
		pvu_create_hash_mode_04(data, hash);
	...
	{
		lenData = pvu_create_data_cw(seed, lenSeed, baseCw, val, seedEcmCw, data);
		pvu_create_hash(data, lenData, hash, hashMode);
...
static uint8_t pvu_sbox(uint8_t *input, uint8_t mode)
{
	uint8_t s_index, bit, last_index, last_bit;
	const uint8_t *Sbox1, *Sbox2, *Sbox3, *Sbox4, *Sbox5, *Sbox6, *Sbox7, *Sbox8, *Sbox9;
	...
	bit = (pvu_get_bit(input[2], 0) << 2) | (pvu_get_bit(input[3], 4) << 1) | (pvu_get_bit(input[5], 3));
	s_index = (pvu_get_bit(input[0], 0) << 3) | (pvu_get_bit(input[2], 6) << 2) | (pvu_get_bit(input[2], 4) << 1) | (pvu_get_bit(input[5], 7));
	last_bit = pvu_get_bit(Sbox1[s_index], 7 - bit);

	bit = (pvu_get_bit(input[5], 0) << 2) | (pvu_get_bit(input[4], 0) << 1) | (pvu_get_bit(input[6], 2));
	s_index = (pvu_get_bit(input[2], 1) << 3) | (pvu_get_bit(input[2], 2) << 2) | (pvu_get_bit(input[5], 5) << 1) | (pvu_get_bit(input[5], 1));
	last_bit = last_bit | (pvu_get_bit(Sbox2[s_index], 7 - bit) << 1);

	bit = (pvu_get_bit(input[6], 0) << 2) | (pvu_get_bit(input[1], 7) << 1) | (pvu_get_bit(input[6], 7));
	s_index = (pvu_get_bit(input[1], 3) << 3) | (pvu_get_bit(input[3], 7) << 2) | (pvu_get_bit(input[1], 5) << 1) | (pvu_get_bit(input[5], 2));
	last_bit = last_bit | (pvu_get_bit(Sbox3[s_index], 7 - bit) << 2);

	bit = (pvu_get_bit(input[1], 0) << 2) | (pvu_get_bit(input[2], 7) << 1) | (pvu_get_bit(input[2], 5));
	s_index = (pvu_get_bit(input[6], 3) << 3) | (pvu_get_bit(input[6], 4) << 2) | (pvu_get_bit(input[6], 6) << 1) | (pvu_get_bit(input[3], 5));
	last_index = pvu_get_bit(Sbox4[s_index], 7 - bit);

	bit = (pvu_get_bit(input[3], 3) << 2) | (pvu_get_bit(input[4], 6) << 1) | (pvu_get_bit(input[3], 2));
	s_index = (pvu_get_bit(input[3], 1) << 3) | (pvu_get_bit(input[4], 5) << 2) | (pvu_get_bit(input[3], 0) << 1) | (pvu_get_bit(input[4], 7));
	last_index = last_index | (pvu_get_bit(Sbox5[s_index], 7 - bit) << 1);

	bit = (pvu_get_bit(input[5], 4) << 2) | (pvu_get_bit(input[4], 4) << 1) | (pvu_get_bit(input[1], 2));
	s_index = (pvu_get_bit(input[2], 3) << 3) | (pvu_get_bit(input[6], 5) << 2) | (pvu_get_bit(input[1], 4) << 1) | (pvu_get_bit(input[4], 1));
	last_index = last_index | (pvu_get_bit(Sbox6[s_index], 7 - bit) << 2);

	bit = (pvu_get_bit(input[0], 6) << 2) | (pvu_get_bit(input[0], 7) << 1) | (pvu_get_bit(input[0], 4));
	s_index = (pvu_get_bit(input[0], 5) << 3) | (pvu_get_bit(input[0], 3) << 2) | (pvu_get_bit(input[0], 1) << 1) | (pvu_get_bit(input[0], 2));
	last_index = last_index | (pvu_get_bit(Sbox7[s_index], 7 - bit) << 3);

	bit = (pvu_get_bit(input[4], 2) << 2) | (pvu_get_bit(input[4], 3) << 1) | (pvu_get_bit(input[1], 1));
	s_index = (pvu_get_bit(input[1], 6) << 3) | (pvu_get_bit(input[6], 1) << 2) | (pvu_get_bit(input[5], 6) << 1) | (pvu_get_bit(input[3], 6));
	last_index = last_index | (pvu_get_bit(Sbox8[s_index], 7 - bit) << 4);

	return (pvu_get_bit(Sbox9[last_index & 0x1F], 7 - last_bit) & 1) ? 1 : 0;
}

static void pvu_decrypt(uint8_t *data, uint32_t length, uint8_t *key, uint8_t sbox)
{
	uint32_t i;
	int32_t j, k;
	...
	for (i = 0; i < length; i++)
	{
		...
		for (j = 7; j >= 0; j--)
		{
			data[i] = pvu_set_bit(data[i], (uint8_t)j,
								  (uint8_t)((pvu_get_bit(curByte, (uint8_t)j) ^ pvu_sbox(key, sbox)) ^ pvu_get_bit(key[0], 7)));
			tmpBit = (uint8_t)(pvu_get_bit(data[i], (uint8_t)j) ^ (pvu_get_bit(key[6], 0)));
			...
			key[0] = (key[0] >> 1);
			key[0] = pvu_set_bit(key[0], 7, tmpBit);
		}
	}
}

static void pvu_expand_des_key(unsigned char *key)
{
	uint8_t i, j, parity;
	uint8_t tmpKey[7];
...
static uint8_t pvu_get_conv_cw_index(uint8_t ecmTag)
{
	switch (ecmTag)
	{
...
static uint16_t pvu_get_seed_iv(uint8_t seedType, uint8_t *ecm)
{
	switch (seedType)
	{
		case PVU_CW_VID:
			return (uint16_t)(((ecm[0x10] & 0x1F) << 3) | 0);

		case PVU_CW_HSD:
			return (uint16_t)(((ecm[0x12] & 0x1F) << 3) | 2);

		case PVU_CW_A1:
			return (uint16_t)(((ecm[0x11] & 0x3F) << 3) | 1);

		case PVU_CW_A2:
			return (uint16_t)(((ecm[0x13] & 0x3F) << 3) | 1);

		case PVU_CW_A3:
			return (uint16_t)(((ecm[0x19] & 0x3F) << 3) | 1);

		case PVU_CW_A4:
			return (uint16_t)(((ecm[0x1A] & 0x3F) << 3) | 1);

		case PVU_CW_UTL:
			return (uint16_t)(((ecm[0x14] & 0x0F) << 3) | 4);

		case PVU_CW_VBI:
			return (uint16_t)((((ecm[0x15] & 0xF8) >> 3) << 3) | 5);
...
static uint8_t pvu_expand_seed(uint8_t seedType, uint8_t *seed)
{
...
static void pvu_calculate_seed(uint8_t seedType, uint8_t *ecm, uint8_t *seedBase,
							   uint8_t *key, uint8_t *seed, uint8_t sbox)
{
	uint16_t tmpSeed;

	tmpSeed = pvu_get_seed_iv(seedType, ecm + 23);

	seed[0] = (uint8_t)((tmpSeed >> 2) & 0xFF);
	seed[1] = (uint8_t)(((tmpSeed & 0x3) << 6) | (seedBase[0] >> 2));
	seed[2] = (uint8_t)((seedBase[0] << 6) | (seedBase[1] >> 2));
	seed[3] = (uint8_t)((seedBase[1] << 6) | (seedBase[2] >> 2));
	seed[4] = (uint8_t)((seedBase[2] << 6) | (seedBase[3] >> 2));
	seed[5] = (uint8_t)((seedBase[3] << 6));

	pvu_decrypt(seed, 6, key, sbox);

	seed[0] = (uint8_t)((seed[1] << 2) | (seed[2] >> 6));
	seed[1] = (uint8_t)((seed[2] << 2) | (seed[3] >> 6));
	seed[2] = (uint8_t)((seed[3] << 2) | (seed[4] >> 6));
	seed[3] = (uint8_t)((seed[4] << 2) | (seed[5] >> 6));
}

static void pvu_calculate_cw(uint8_t seedType, uint8_t *seed, uint8_t csaUsed,
							 uint8_t *convolvedCw, uint8_t *cw, uint8_t *baseCw,
							 uint8_t *seedEcmCw, uint8_t hashModeCw, uint8_t needsUnmasking,
							 uint8_t xorMode, int modeCW, uint8_t *ecmBody)
{
	int32_t k;
	uint8_t seedLength, val = 0;

	seedLength = pvu_expand_seed(seedType, seed);
		...
		(modeCW == 0x03) || (modeCW == 0x04)))
	{
		...
		pvu_create_cw(seed, seedLength, baseCw, val, seedEcmCw, cw, csaUsed, hashModeCw, modeCW, ecmBody);

		if (csaUsed)
		{
			for (k = 0; k < 8; k++)
			{
				cw[k] = cw[k] ^ convolvedCw[k];
			}
	...
	else
	{
		...
		else
		{
			...
			pvu_expand_des_key(cw);
		}
	}
}

// --- Pesquisa de chaves 'P' ---
...
// --- ECM PowerVU (modo CSA; dw[0..7] ou dw[8..15] consoante par/ímpar) ---

int cccam_emu_powervu_ecm(uint16_t caid, uint16_t sid, const uint8_t *ecm_in,
                          uint16_t ecm_in_len, uint8_t *dw)
{
	uint32_t i, j, k;
	uint32_t ecmCrc32;
	uint16_t ecmLen = ecm_in_len;
	uint16_t nanoLen, channelId, ecmSrvid;
	uint8_t keyIndex, sbox, decrypt_ok, hashModeCw = 0, needsUnmasking, xorMode;
	uint8_t nanoCmd, nanoChecksum, keyType, fixedKey, oddKey, bid, csaUsed, modeCW = 0, offsetBody;
	uint8_t ecmKey[7], tmpEcmKey[7], seedBase[4], baseCw[7], seed[8], cw[8], convolvedCw[8][8];
	uint8_t ecmPart1[14], ecmPart2[27];
	uint8_t unmaskedEcm[512], seedEcmCw[16];
	uint8_t *ecm;

	(void)caid;
	(void)sid;

	if (ecm_in == NULL || dw == NULL || ecmLen < 7 || ecmLen > sizeof(unmaskedEcm))
	{
		return CCCAM_EMU_NOT_SUPPORTED;
	}

	memcpy(unmaskedEcm, ecm_in, ecmLen);
	ecm = unmaskedEcm;
	...
	if (needsUnmasking)
	{
		hashModeCw = pvu_unmask_ecm(ecm, seedEcmCw, &modeCW);
	}

	ecmCrc32 = pvu_b2i(4, ecm + ecmLen - 4);

	if (pvu_ccitt32_crc(ecm, ecmLen - 4) != ecmCrc32)
	{
		return CCCAM_EMU_CHECKSUM_ERROR;
	}
	ecmLen -= 4;

	memset(convolvedCw, 0, sizeof(convolvedCw));

	for (i = 3; i + 3 < ecmLen; )
	{
		nanoLen = (uint16_t)(((ecm[i] & 0x0F) << 8) | ecm[i + 1]);
		i += 2;
		...
		switch (nanoCmd)
		{
			case 0x27:
				...
				keyType = pvu_get_conv_cw_index(ecm[i + 4]);
	...
	for (i = 3; i + 3 < ecmLen; )
	{
		...
		switch (nanoCmd)
		{
			case 0x20:
			{
				...
				offsetBody = (uint8_t)(i + 4 + ecm[i + 3]);
				i += ecm[i + 3];

				csaUsed = pvu_get_bit(ecm[i + 7], 7);
				fixedKey = !pvu_get_bit(ecm[i + 6], 5);
				oddKey = pvu_get_bit(ecm[i + 6], 4);
				xorMode = pvu_get_bit(ecm[i + 6], 0);
				bid = (pvu_get_bit(ecm[i + 7], 1) << 1) | pvu_get_bit(ecm[i + 7], 0);
				sbox = pvu_get_bit(ecm[i + 6], 3);

				keyIndex = (fixedKey << 3) | (bid << 2) | oddKey;
				channelId = pvu_b2i(2, ecm + i + 23);
				ecmSrvid = (uint16_t)((channelId >> 4) | ((channelId & 0xF) << 12));

				// Apenas modo CSA (o modo DES/AES requer o stream server)
				if (!csaUsed)
				{
					return CCCAM_EMU_NOT_SUPPORTED;
				}
				...
				do
				{
					if (!pvu_get_ecm_key(ecmKey, ecmSrvid, keyIndex) &&
						!pvu_get_ecm_key(ecmKey, channelId, keyIndex))
					{
						cccam_log(LOG_DEBUG, "EMU PowerVU: chave nao encontrada P ****%04X %02X",
								  ecmSrvid, keyIndex);
						return CCCAM_EMU_KEY_NOT_FOUND;
					}

					pvu_decrypt(ecm + i + 8, 14, ecmKey, sbox);
					...
					pvu_decrypt(ecm + i + 27, 27, ecmKey, sbox);
				...
				pvu_calculate_seed(PVU_CW_VID, ecm + i, seedBase, ecmKey, seed, sbox);

				memcpy(baseCw, ecm + i + 6 + 8, 7);

				pvu_calculate_cw(PVU_CW_VID, seed, csaUsed, convolvedCw[PVU_CW_VID],
								 cw, baseCw, seedEcmCw, hashModeCw, needsUnmasking,
				...
				cccam_log(LOG_DEBUG, "EMU PowerVU: CW obtida (chave P ****%04X %02X)",
						  ecmSrvid, keyIndex);
				return CCCAM_EMU_OK;
			}
